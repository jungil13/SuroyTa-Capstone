  ...
  try {
    const [posts] = await db.promise().query(`
      SELECT 
        ...
        p.hidden, -- Include the hidden column
        COUNT(l.like_id) AS likeCount  
      FROM posts p
      JOIN categories c ON p.category_id = c.category_id
      JOIN users u ON p.user_id = u.user_id
      LEFT JOIN likes l ON p.post_id = l.post_id  
      GROUP BY p.post_id, u.username, c.name, p.user_id, u.bio, p.hidden  -- Include hidden in GROUP BY
      ORDER BY p.created_at DESC
    `);

    console.log(posts); // Log the posts to verify that hidden is included
