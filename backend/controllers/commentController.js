...
exports.updatePromoComment = async (req, res) => {
  const { promotionid } = req.params;
  const { content } = req.body;
  const userId = req.user.user_id; // Retrieved from the authenticated user middleware
  ...
  try {
    ...
    const comment = await Comment.getCommentByPromotionAndUser(promotionid, userId);
    ...
    // Update the comment
    const result = await Comment.updatePromoComment(promotionid, userId, content);
...
exports.deletePromoComment = async (req, res) => {
  const { promotionid } = req.params;
  const userId = req.user.user_id; // Retrieved from the authenticated user
  ...
  try {
      ...
      // Delete the comment if ownership is verified
      const result = await Comment.deletePromoComment(promotionid, userId);
      if (result.affectedRows === 0) {
